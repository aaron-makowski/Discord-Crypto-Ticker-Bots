...
async def background_loop():
      ...
      while not client.is_closed():
            try:
                  USD_ = requests.get(url='https://api.binance.com/api/v1/ticker/24hr?symbol=ETHUSDT')
                  USD_.close()
                  USD = USD_.json()

                  USD_price = round(float(USD["lastPrice"]), 2)
                  percent = round(float(USD["priceChangePercent"]), 1)

                  BTC_ = requests.get(url='https://api.binance.com/api/v1/ticker/24hr?symbol=ETHBTC')
                  BTC_.close()
                  BTC = BTC_.json()
                  
                  BTC_price = float(BTC["lastPrice"])

                  playing = '$'+str(USD_price)+' '+str(BTC_price)
