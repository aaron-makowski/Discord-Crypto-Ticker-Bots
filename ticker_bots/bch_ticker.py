...
async def background_loop():
      ...
      while not client.is_closed():
            try:
                  USD_ = requests.get(url='https://api.binance.com/api/v1/ticker/24hr?symbol=BCHABCUSDT')
                  USD_.close()
                  USD = USD_.json()
                  ...
                  BTC_ = requests.get(url='https://api.binance.com/api/v1/ticker/24hr?symbol=BCHABCBTC')
                  BTC_.close()
                  BTC = BTC_.json()
